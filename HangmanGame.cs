...
namespace NadekoBot.Modules.Games.Hangman
{
    public sealed class HangmanGame
    {
        ...
        public HangmanGame(HangmanTerm term)
        {
            ...
            _remaining = _word
                .ToLowerInvariant()
                .Where(x => Regex.IsMatch(x.ToString(), @"[А-яёЁA-z]"))
        ...
        private string GetScrambledWord()
        {
            ...
            for (var i = 0; i < _word.Length; i++)
            {
                ...
                if (ch == ' ')
                    output[i*2] = ' ';
                if (!Regex.IsMatch(ch.ToString(), @"[А-яёЁA-z]") || !_remaining.Contains(char.ToLowerInvariant(ch)))
                    output[i*2] = ch;
                else
                    output[i*2] = '_';
        ...
        public State Guess(string guess)
        {
            ...
            if (!Regex.IsMatch(charGuess.ToString(), @"[А-яёЁA-z]"))
                return GetState(GuessResult.NoAction);
